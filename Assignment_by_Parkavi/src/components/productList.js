...
class ProductList extends Component {
    ...
    Table = (currentTodos) => {
        return (<table border="1" className="tableBorder">
            ...
            {(currentTodos && currentTodos.map(prod => {
    ...
    tableDisplay = (products, currentPageNum, contentNumber) => {
        ...
        const currentTodos = (products && products.slice(indexOfFirstTodo,
            indexOfLastTodo));
        return (
    ...
    paginate = (products, contentNumber) => {
        const pageNumbers = [];
        for (let i = 1; i <= Math.ceil(products && products.length / contentNumber); i++) {
            pageNumbers.push(i);
        }
